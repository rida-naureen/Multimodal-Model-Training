...
#          shape: [T_v, 2048]  (variable length frames, 15 FPS)
# ============================================================
...
def embed_frames(frames):
    """
    RGB face crop list → numpy [T_v, 2048]
    We bypass the model mean pooling in `get_visual_encoder().encode` 
    because our architecture supports dynamic visual sequence masking.
    """
    if not frames:
    ...
    for rgb in frames:
        ...
        # Same transformation logic found inside get_visual_encoder()
        if rgb.dtype != np.uint8:
            rgb = (rgb * 255).astype(np.uint8)
        tensors.append(encoder.transform(rgb))
        
    if not tensors:
        return np.zeros((1, 2048), dtype=np.float32)
        
    batch = torch.stack(tensors).to(encoder.device)         # [T_v, 3, 224, 224]
    ...
    with torch.no_grad():
        for i in range(0, batch.shape[0], chunk_size):
            ...
            feats = encoder.model(chunk)             # [chunk, 2048]
            ...
            for utt_id, (start_sec, end_sec) in timestamps.items():
                ...
                try:
                    ...
                    if faces is None:
                        errors += 1
                        continue
                    emb = embed_frames(faces)           # [T_v, 2048]
